...
/* i18n */
var I={
fr:{sub:"Envoie ton problème — l'IA détecte, résout et vérifie",tw:"Écrire",tp:"Photo",tf:"Fichier PDF",yp:"Ton problème",tph:"Ex : Calculer l'intégrale de x² sin(x) dx entre 0 et π…",pd:"Glisse ta photo ici ou clique",ph:"Prends en photo ton exercice",fd:"Glisse ton PDF ici ou clique",fh:"Upload un TD, examen, poly…",instr:"Dis à l'IA ce que tu veux",opt:"optionnel",instrph:"Ex : Résous l'exercice 3 question b, utilise la méthode de Gauss, détaille chaque étape…",sv:"Résoudre ✦",st:"Arrêter",solving:"Résolution…",stt:"✦ Solution détaillée",cp:"Copier",cpd:"Copié ✓",fq:"Cette solution est-elle correcte ?",fy:"✓ Correcte",fn:"✗ Incorrecte",vc:"Vérification…",vok:"✓ Solution validée",vw:"⚠ Incertitudes détectées",rh:"🔍 Recherche et correction",ct:"✦ Solution corrigée (recherche web)",hi:"Historique",et:"~15-30s",ep2:"~20-40s",ef:"~25-45s",td:"Terminé en",stopped:"Résolution arrêtée",ftk:"✓ Merci ! Correcte.",fsr:"🔍 Recherche web…",fns:"✓ Nouvelle solution ci-dessous",r1:"Recherche…",r2:"Analyse…",r3:"Correction…",r1d:"✓ Recherche",r2d:"✓ Analyse",r3d:"✓ Corrigé",rem:"Retirer",pfx:"Résous :\n",pfxl:"Résous (LaTeX) :\n",pi:"Résous tous les exercices de cette image.",fi:"Résous tous les exercices de ce document.",swb:"Fichier volumineux (SZ). L'envoi pourrait échouer.",swh:"Fichier trop volumineux (SZ). Prends une photo de la page.",ef1:"Erreur réseau. Vérifie ta connexion.",ef2:"Fichier trop volumineux. Prends une photo de la page.",eg:"Erreur",ful:"Encore une question ? Un autre exercice ?",fuph:"Ex : Maintenant résous l'exercice 4, ou bien : Explique-moi l'étape 2 plus en détail…",fusend:"Envoyer"},
en:{sub:"Send your problem — AI detects, solves and verifies",tw:"Write",tp:"Photo",tf:"PDF File",yp:"Your problem",tph:"E.g.: Calculate the integral of x² sin(x) dx from 0 to π…",pd:"Drag photo here or click",ph:"Take a photo of your exercise",fd:"Drag PDF here or click",fh:"Upload problem set, exam…",instr:"Tell the AI what you need",opt:"optional",instrph:"E.g.: Solve exercise 3 question b, use Gaussian elimination, detail each step…",sv:"Solve ✦",st:"Stop",solving:"Solving…",stt:"✦ Detailed Solution",cp:"Copy",cpd:"Copied ✓",fq:"Is this solution correct?",fy:"✓ Correct",fn:"✗ Incorrect",vc:"Verifying…",vok:"✓ Solution validated",vw:"⚠ Uncertainties detected",rh:"🔍 Searching & correcting",ct:"✦ Corrected (web search)",hi:"History",et:"~15-30s",ep2:"~20-40s",ef:"~25-45s",td:"Completed in",stopped:"Stopped",ftk:"✓ Thanks! Correct.",fsr:"🔍 Web search…",fns:"✓ New solution below",r1:"Searching…",r2:"Analyzing…",r3:"Correcting…",r1d:"✓ Searched",r2d:"✓ Analyzed",r3d:"✓ Corrected",rem:"Remove",pfx:"Solve:\n",pfxl:"Solve (LaTeX):\n",pi:"Solve all exercises in this image.",fi:"Solve all exercises in this document.",swb:"Large file (SZ). May fail.",swh:"File too large (SZ). Take a photo instead.",ef1:"Network error. Check your connection.",ef2:"File too large. Take a photo of the page.",eg:"Error",ful:"Another question? Another exercise?",fuph:"E.g.: Now solve exercise 4, or: Explain step 2 in more detail…",fusend:"Send"}
};
var lang='fr';
...
/* Chips */
var CE={fr:["Exercice 2 question b","Méthode de Gauss","Détailler chaque étape","Par récurrence","Expliquer simplement","Partie A uniquement"],en:["Exercise 2 question b","Gaussian elimination","Detail each step","By induction","Explain simply","Part A only"]};
...
function solve(){
    ...
    if(M==='text'){
        ...
        var userMsg=document.getElementById('pci').value.trim();
        var ins=userMsg||t('pi');
        cons=userMsg;
        ...
        var userMsg2=document.getElementById('fci').value.trim();
        var ins2=userMsg2||t('fi');
        cons=userMsg2;
    ...
    .then(function(data){
        ...
        document.getElementById('fu').classList.add('vis');
        verify(txt,uc,myG);
    })
    .catch(function(err){
        if(myG!==GEN)return;
...
/* Follow-up */
function followUp(){
    var msg=document.getElementById('fui').value.trim();
    if(!msg)return;
    var btn=document.getElementById('sb'),xb=document.getElementById('xb');
    // Hide previous feedback/verify/retry but keep solution visible
    document.getElementById('vb').className='vbar';
    document.getElementById('fb').classList.remove('vis');
    document.getElementById('fbs').style.display='flex';
    document.getElementById('fd').className='fdone';
    ['s2','rs'].forEach(function(x){document.getElementById(x).style.display='none'});
    document.getElementById('eb').style.display='none';
    document.getElementById('xbar').classList.remove('vis');
    document.getElementById('fu').classList.remove('vis');

    // Build content: same file/photo + new message
    var uc=[];
    if(M==='photo'&&PD){
        uc=[{type:"image",source:{type:"base64",media_type:PD.mt,data:PD.b64}},{type:"text",text:msg}];
    }else if(M==='file'&&FD){
        uc=[{type:"document",source:{type:"base64",media_type:"application/pdf",data:FD.b64}},{type:"text",text:msg}];
    }else{
        uc=[{type:"text",text:msg}];
    }
    ...
    apiCall({model:"claude-sonnet-4-20250514",max_tokens:4096,system:getSys(msg),messages:[{role:"user",content:uc}]})
    .then(function(data){
        if(myG!==GEN)return;
        var txt=getText(data);
        if(!txt)throw new Error(lang==='fr'?"Réponse vide":"Empty response");
    ...
    .catch(function(err){
        if(myG!==GEN)return;
        timerOff(false);xb.classList.remove('vis');
        var emsg=(err.message==='Failed to fetch'||err.message==='timeout')?t('ef1'):t('eg')+': '+err.message;
        document.getElementById('eb').textContent=emsg;
        document.getElementById('eb').style.display='block';
        btn.classList.remove('ld');btn.disabled=false;
        document.getElementById('fu').classList.add('vis');
    });
...
document.getElementById('fui').addEventListener('keydown',function(e){if(e.key==='Enter'&&(e.ctrlKey||e.metaKey))followUp()});
